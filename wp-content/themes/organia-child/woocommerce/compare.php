...
$table_text = get_option( 'yith_woocompare_table_text', __( 'Compare products', 'organia' ) );
...
<head>
	...
	<link rel="stylesheet" href="<?php echo esc_url( $this->stylesheet_url() ); ?>" type="text/css"/> <?php // phpcs:ignore ?>
        <link rel="stylesheet" href="<?php echo esc_url(get_template_directory_uri().'/assets/css/compare.css' ); ?>" type="text/css"/>
...
<!-- START BODY -->
<body <?php body_class( 'woocommerce yith-woocompare-popup organiaCompare' ); ?>>
...
<h1 class="organCHead">
	...
	if ( ! $is_iframe ) :
		?>
		<a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'organia' ); ?></a><?php endif; ?>
</h1>
	...
	>
		...
		<?php if ( empty( $products ) ) : ?>
			...
			<tr class="no-products">
				<td><?php esc_html_e( 'No products added in the compare table.', 'organia' ); ?></td>
			</tr>
			...
			<tr class="remove">
				...
				foreach ( $products as $product_id => $product ) :
					...
					<td class="<?php echo esc_attr( $product_class ); ?>">
						...
						?>
						"
								data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php esc_html_e( 'Remove', 'organia' ); ?>
